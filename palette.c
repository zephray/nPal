...
--*/
{
   ...
   BYTE        buf[1536];//PAL_LARGE
   INT                   i;
   FILE                 *fp;

   fp = UTIL_OpenRequiredFile("pat.mkf.tns");
...
--*/
{
   ...
   SDL_Color      newpalette[256];//PAL_LARGE
   ...
   // Start fading out...
   //
   time = SDL_GetTicks() + iDelay * 10 * 50;
   ...
   while (TRUE)
   {
      ...
      for (i = 0; i < 256; i++)
      {
         ...
         newpalette[i].b = (palette[i].b * j) >> 6;
		  //printf("New palette RGB=(%d,%d,%d)",newpalette[i].r,newpalette[i].g,newpalette[i].b);
      }
