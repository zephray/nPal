...
--*/
{
   ...
   //
   gpScreen = SDL_CreateRGBSurface(gpScreenReal->flags & ~SDL_SWSURFACE, 320, 200, 8,
...
--*/
{
   ...
   if (lpRect != NULL)
   {
      dstrect.x = (SHORT)(lpRect->x);
      dstrect.y = (SHORT)(screenRealY + lpRect->y);
      dstrect.w = (WORD)(lpRect->w);
      dstrect.h = (WORD)(lpRect->h);
...
--*/
{
   ...
   {
      fp = fopen(va("%sscrn%.4d.bmp.tns", PAL_PREFIX, iNumBMP), "rb");
      if (fp == NULL)
      {
         break;
      }
   ...
   // Save the screenshot.
   //
   SDL_SaveBMP(gpScreenReal, va("%sscrn%.4d.bmp.tns", PAL_PREFIX, iNumBMP));
}
